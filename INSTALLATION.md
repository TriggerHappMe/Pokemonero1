...
```
```
git clone https://github.com/TriggerHappMe/pokemonero.git
```
# Permissions

After files are copied we need to give them permissions.
```
sudo chown -R www-data:www-data /var/www/pokemonero/public
```
```
sudo chmod 755 /var/www
```
```
sudo chmod -R 755 /var/www/pokemonero/bootstrap/cache
```
```
sudo chmod -R 755 /var/www/epokemonero/storage
```
```
sudo chown -R $USER:www-data /var/www/pokemonero/storage
```
```
sudo chown -R $USER:www-data /var/www/pokemonero/bootstrap/cache
```
```
sudo chmod -R 775 /var/www/pokemonero/storage
```
```
sudo chmod -R 775 /var/www/pokemonero/bootstrap/cache
```

Make this folder: (used for product pictures):
```
sudo mkdir /var/www/pokemonero/storage/public/
sudo mkdir /var/www/pokemonero/storage/public/products
```

And give it permissions
```
sudo chmod -R 755 /var/www/pokemonero/storage/public/products
```
```
sudo chgrp -R www-data /var/www/pokemonero/storage/public/products
```
```
...
server {
        ...
        listen 443;
        listen [::]:443;
root /var/www/pokemonero/public;
